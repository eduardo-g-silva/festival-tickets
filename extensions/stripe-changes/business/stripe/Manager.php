...
class Manager extends \common\modules\payment\business\Manager
{
    ...
    public function processCharge($postData)
    {
        ...
        $checkout       = ApplicationUtil::getCheckout();
        $order          = $checkout->order;
        $intent = \Stripe\PaymentIntent::retrieve($postData['intentId']);
        return $this->processOrderUpdate($intent->jsonSerialize(), $order);
    }
     ...
     * @return void
     */
    public function processOrderUpdate($intentResponse, $order)
    {
        $response               = $intentResponse['charges']['data'][0];
